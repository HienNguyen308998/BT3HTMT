...
namespace HTMTTuan3
{
    class Program
    {
        ...
        static string DecToHex(int DecNumber)
        {
            if (DecNumber == 0)
            {
                return "0";
        ...
        static int HexToDec(string HexNumber)
        {
            ...
            for (int i = HexNumber.Length - 1, pow = 0; i >= 0; i--, pow++)
            {
                ...
                DecResult += HexIndex *(int)Math.Pow(16, pow);
            }
            return DecResult;
        }
        ...
        static int BinToDec(string BinNumber)
        {
            int DecResult = 0;
            for (int i=BinNumber.Length-1, pow = 0;i>=0;pow--,i++)
            {
                DecResult += (int)Math.Pow(2, pow);
            }    
            return DecResult;
        }
        ...
        static string BinToHex(string BinNumber)
        {
            ...
            for (int i = BinNumber.Length - 1, pow = 0; i >= 0; i--, pow++)
                ...
                DecResult += digit*(int)Math.Pow(2, pow);
            }
            //Chuyển đổi số từ hệ 10 về hệ 16
            if (DecResult == 0)
            {
        ...
        static void Main(string[] args)
        {
            ...
            int DecResult2 = HexToDec(input2);
            ...
            int DecResult3 = BinToDec(input3);
